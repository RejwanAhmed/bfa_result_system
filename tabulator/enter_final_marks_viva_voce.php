    ...
    if(isset($_POST['submit']))
    {
        ...
        if($count_error==0 && $previous_current_cgpa_difference==0)
        {
            for($i=0;$i<sizeof($_POST['id']);$i++)
            {
                ...
                if(ceil($total_final_marks*2)<40)
                {
                ...
                $update_result = "UPDATE `result` SET `total_final_marks` = '$total_final_marks', `improvement_eligibility` = '$improvement_eligibility' WHERE `id` = '$res_id'";
            ...
            if($exam_committee_status!=3 && sizeof($students_roll)>1)
            {
                ...
                for($i=0;$i<sizeof($_POST['new_student_id']);$i++)
                {
                    ...
                    if(ceil($new_student_total_final_marks*2)<40)
                    {
                        $improvement_eligibility = "Y";
